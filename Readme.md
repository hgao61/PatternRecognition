...
This is an agent that can address a specific intelligence test. The AI agent can answer 2x1 and 2x2 visual analogy problems based on visual representations. The problems were based on those found in the Raven's Progressive Matrices Human Intelligence test. The agent is trained based on 40 samples. 
...
3. Compute a set of possible solutions 
   1. Calculate possible transformations from A to B (transforms) 
   2. Calculate possible transformations from A to C (analogies)   
   3. Combine analogies with transformations to build a set of possible solutions. 
   4. Sort possible solutions based on overall cost 
4. Test each solutions against the answer set in ascending order of cost 
   1. If a solution matches one of the answers return the solution. 
